...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, DepthwiseConvBlock, autopad
...
class DepthwiseConvBlock(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=0, d=1, freeze_bn=False, bn_momentum=0.9, bn_eps=1e-5):
        super(DepthwiseConvBlock, self).__init__()
        self.depthwise = nn.Conv2d(
            c1, c1, kernel_size=k, stride=s, padding=p, dilation=d, groups=c1, bias=False
        )
...
class BiFPNBlock(nn.Module):
    ...
    def __init__(self, feature_size=64, epsilon=0.0001):
        ...
        self.w1 = nn.Parameter(torch.ones(2, 4))  # Shape: [2, 4], initialized to 1
        self.w2 = nn.Parameter(torch.ones(3, 4))  # Shape: [3, 4], initialized to 1
    ...
    def forward(self, inputs):
        ...
        # Calculate Top-Down Pathway with bilinear interpolation
...
class BiFPN(nn.Module):
    def __init__(self, size, feature_size=64, num_layers=2, epsilon=0.0001):
        ...
        # p7 is computed by applying SiLU followed by a 3x3 stride-2 conv on p6
        self.p7 = ConvBlock(feature_size, feature_size, kernel_size=3, stride=2, padding=1)

        bifpns = []
        for _ in range(num_layers):
            bifpns.append(BiFPNBlock(feature_size=feature_size, epsilon=epsilon))
    ...
    def forward(self, inputs):
        ...
        return self.bifpn(features)  # Returns [p3_out, p4_out, p5_out, p6_out, p7_out]
